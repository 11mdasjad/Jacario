...
<div class="max-w-3xl mx-auto space-y-6" x-data="{
    ...
    <div class="flex items-center justify-between pb-4 border-b border-zinc-200">
        <div>
            <h1 class="text-xl font-serif-luxury font-bold text-zinc-950">Add Promotional Hero Banner</h1>
            <p class="text-xs text-zinc-500 mt-0.5">Configure imagery, typography, badge, and CTA for the storefront carousel</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.banners.store') }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="p-6 sm:p-8 bg-white rounded-2xl border border-zinc-200 shadow-xs space-y-5">
            ...
            <div>
                ...
                <input type="text" name="title" value="{{ old('title') }}" required placeholder="e.g. The Polo, Perfected." class="w-full text-xs bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-zinc-900 focus:outline-none focus:border-black">
            ...
            <div>
                ...
                <textarea name="subtitle" rows="2" placeholder="e.g. Premium Polo T-Shirts designed for effortless everyday style." class="w-full text-xs bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-zinc-900 focus:outline-none focus:border-black">{{ old('subtitle') }}</textarea>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="badge_text" value="{{ old('badge_text') }}" placeholder="e.g. ✦ The Haute Collection" class="w-full text-xs bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-zinc-900 focus:outline-none focus:border-black">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-zinc-700 mb-1">Carousel Sort Order *</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $nextOrder) }}" required min="1" max="99" class="w-full text-xs bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-zinc-900 font-bold focus:outline-none focus:border-black">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-zinc-700 mb-1">CTA Button Text *</label>
                    <input type="text" name="cta_text" value="{{ old('cta_text', 'Shop Now') }}" required placeholder="e.g. Shop Now, Explore Collection" class="w-full text-xs bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-zinc-900 font-bold focus:outline-none focus:border-black">
                </div>

                <div>
                    <label class="block text-xs font-semibold text-zinc-700 mb-1">CTA Destination URL *</label>
                    <input type="text" name="cta_url" value="{{ old('cta_url', '/shop') }}" required placeholder="e.g. /shop or /shop?collection=new-arrivals" class="w-full text-xs bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-zinc-900 focus:outline-none focus:border-black">
                </div>
                ...
                <div>
                    ...
                    <select name="position" class="w-full text-xs bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-zinc-900 font-bold focus:outline-none focus:border-black">
                        <option value="hero" {{ old('position', $defaultPosition ?? 'hero') === 'hero' ? 'selected' : '' }}>✨ Homepage Hero Carousel (Full Bleed)</option>
                        <option value="shop" {{ old('position', $defaultPosition ?? 'hero') === 'shop' ? 'selected' : '' }}>🛍️ Shop Catalog Running Carousel (3-Banner Header)</option>
                    </select>
                </div>
                ...
                <div>
                    ...
                    <select name="text_alignment" class="w-full text-xs bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-zinc-900 focus:outline-none focus:border-black">
                        <option value="left" {{ old('text_alignment', 'left') === 'left' ? 'selected' : '' }}>Left Aligned (Editorial)</option>
                        <option value="center" {{ old('text_alignment') === 'center' ? 'selected' : '' }}>Center Aligned (Cinematic)</option>
                        <option value="right" {{ old('text_alignment') === 'right' ? 'selected' : '' }}>Right Aligned (Sartorial)</option>
                    </select>
                </div>

                <div class="sm:col-span-2">
                    <label class="block text-xs font-semibold text-zinc-700 mb-1">Mobile Image URL (Optional)</label>
                    <input type="url" name="mobile_image_url" value="{{ old('mobile_image_url') }}" placeholder="https://... (falls back to desktop image)" class="w-full text-xs bg-zinc-50 border border-zinc-200 rounded-xl p-3 text-zinc-900 focus:outline-none focus:border-black">
                </div>
            </div>
            ...
            <div class="pt-2">
                <label class="flex items-center space-x-2 text-xs font-bold text-zinc-800 cursor-pointer">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="w-4 h-4 rounded text-black focus:ring-black">
        ...
        <div class="p-6 sm:p-8 bg-white rounded-2xl border border-zinc-200 shadow-xs space-y-4">
            ...
            <div class="aspect-[16/9] rounded-2xl bg-zinc-100 border border-zinc-200 overflow-hidden relative flex items-center justify-center">
                ...
                <template x-if="!imageUrl">
                    <div class="text-center p-6 text-zinc-400">
                        ...
                        <p class="text-[10px]">Recommended aspect ratio: 16:9 or 21:9 (1800 x 900 px)</p>
                    </div>
                </template>
            </div>
            ...
            <div>
                <label class="block text-xs font-semibold text-zinc-700 mb-1">Option A: Direct Image URL</label>
            ...
            <div>
                <label class="block text-xs font-semibold text-zinc-700 mb-1">Option B: Or Upload Image File from Device</label>
