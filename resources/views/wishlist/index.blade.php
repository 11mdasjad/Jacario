    ...
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            @foreach($wishlistItems as $item)
                ...
                <div class="bg-white rounded-xl border border-zinc-200 p-4 flex flex-col justify-between space-y-4 relative group">
                    
                    <form method="POST" action="{{ route('wishlist.toggle') }}" class="absolute top-3 right-3 z-10">
                        @csrf
                        <input type="hidden" name="product_id" value="{{ $product->id }}">
                        <button type="submit" class="w-8 h-8 rounded-full bg-white/90 shadow-sm border border-zinc-100 flex items-center justify-center text-rose-600 hover:scale-110 transition-transform" title="Remove from wishlist">
                            <svg class="w-4 h-4 fill-rose-600" viewBox="0 0 24 24"><path d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>
                        </button>
                    </form>

                    <a href="{{ route('products.show', $product->slug) }}" class="aspect-[4/5] bg-zinc-50 rounded-lg p-4 flex items-center justify-center overflow-hidden">
                        <img src="{{ $product->primaryImage ? $product->primaryImage->url : asset('images/placeholder-polo.svg') }}" alt="{{ $product->name }}" class="w-full h-full object-contain group-hover:scale-105 transition-transform duration-300">
                    </a>

                    <div class="space-y-1">
                        <p class="text-[10px] font-bold uppercase tracking-wider text-zinc-400">{{ $product->category->name }}</p>
                    ...
                    <form method="POST" action="{{ route('wishlist.move-to-cart', $item->id) }}">
                        @csrf
                        <button type="submit" class="w-full py-2.5 bg-zinc-900 hover:bg-black text-[#DFCAAB] text-xs font-bold uppercase tracking-wider rounded-lg transition-colors flex items-center justify-center space-x-1.5">
